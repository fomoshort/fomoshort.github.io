...
const main = function() {
  ...
  async function initGame(object) { 
    await updateFomoShortVault(object);	  
    await updateFomoShortBuyPrice(object);
    await updateFomoShortTime(object);	 
    await initFomoShortBuyButton(object);
    await initFomoShortReinvestButton(object);	  
  }	
  ...
  async function initFomoQuick() {
    let fomoShortSettings = { address:'', abi:JSON.parse('') };
    let fomoShort = fomoShortObject(fomoShortSettings);
    await initGame(fomoShort);
  }	
