...
const hourglass = function() {
  ...
  async function sellTokens() {
    ...
    let hourglassContract = web3.eth.contract(hourglass.abi).at(hourglass.address);	    
    let value = math.toFixed(parseFloat(new BigNumber($('#p3dAmount').val()).mul(Math.pow(10,18))));	  
    let data = hourglassContract.sell.getData(value);
  ...
  async function buyTokens() {
    ...
    let value = math.toFixed(parseFloat(new BigNumber($('#p3dAmount').val()).mul(Math.pow(10,18)).mul(keyPrice)));	  
    await tx.sendTransaction({from:userAddress, to:hourglass.address, data:data, value:value});	  
  }
  return {getSellPrice, getBuyPrice, sellTokens, buyTokens};			     
}	
const game = function(_gameSettings) {
...
const main = function() {
  ...
  async function initFomoShort() { 
    ...
    let fomoShort = fomoShortObject(fomoShortSettings);
    await updateFomoShortVault(fomoShort);	  
    await updateFomoShortBuyPrice(fomoShort);
    await updateFomoShortTime(fomoShort);	 
    await initFomoShortBuyButton(fomoShort);
    await initFomoShortReinvestButton(fomoShort);	  
  }	
  async function init() {
    await initHourGlass();	  
    await initFomoShort();	  
