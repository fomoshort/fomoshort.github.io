...
const playerbookObject = function() {
  ...
  function getPlayerName(id) {
	...
	if(!err) {
	  resolve("https://fomoshort.github.io/" + web3.toAscii(result).replace(/\0.*$/g,''));		
        }	       
        else { reject(err) }      
      });	      
    });	    
...
const hourglassObject = function() {
  ...
  async function sellTokens(amount) {
    ...
    await tx.sendTransaction({from:userAddress, to:hourglass.address, data:data, gasPrice:5*1e9});	  
  }	
...
const main = function() {
  ...
  async function updatePlayerInfo(object) {
    setInterval(async function() {
      object.name === "fomoShort" ? (playerInfo = await object.getPlayerInfo(), $('#fomoShortPlayerKeys').text((parseFloat(playerInfo[2])/1e18).toString()) :
  ...
  async function updateBuyPrice(object) {
    setInterval(async function() {	    
      ...
      $('#fomoShortKeysPrice').text((fomoShortKeysPrice/1e18).toString() + " ETH")) :
      (fomoQuickKeysPrice = await object.getBuyPrice(),
      $('#fomoQuickKeysPrice').text((fomoQuickKeysPrice/1e18).toString() + " ETH"));
    }, 3000);	    
  }
